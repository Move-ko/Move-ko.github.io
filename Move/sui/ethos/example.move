module ethos::example {
    ...
    public entry fun mint(ctx:&mut TxContext){
        ...
        transfer::transfer(ethos_exmample,tx_context::sender(ctx));
    }

    public entry fun modify(ethos_exmape:&mut EthosExample,new_message:vector<u8>) {
        ethos_example.message= string::utf8(new_message);
        ethos_example.url = url(ethos_exmample.message);
    }

    public entry fun burn(ethos_example:EthosExample) {
        let EthosExample {id ,name:_,descript:_,message:_,url:_}= ethos_example;
        object::delete(id);
