  ...
  return (
    <Grid container>
      ...
      <Grid xs={12} md={12} sx={{ marginTop: "30px" }}>
        <Box sx={{ width: "100%", textAlign: "left" }}>
          <Typography variant="body1" gutterBottom>
            Move에는 불변 & 및 가변 &mut의 두 가지 유형의 참조가 있습니다.
            변경할 수 없는 참조는 읽기 전용이며 기본 값(또는 해당 필드)을 수정할
            수 없습니다. 변경 가능한 참조는 해당 참조를 통한 쓰기를 통한 수정을
      ...
      <Grid xs={12} md={12} sx={{ marginTop: "30px" }}>
        <Box sx={{ width: "100%", textAlign: "left" }}>
          <Typography variant="body1" gutterBottom>
            ...
            곳에서는 "표현식 e에는 T 유형이 있음"에 대해 표기법 e: T를
            사용합니다.
          </Typography>
        </Box>
      </Grid>
      ...
      <Grid xs={12} md={12} sx={{ marginTop: "30px" }}>
        <Box sx={{ width: "100%", textAlign: "left" }}>
          <Typography variant="body1" gutterBottom>
            &e.f 및 &mut e.f 연산자는 구조체에 대한 새 참조를 만들거나 기존
            참조를 확장하는 데 모두 사용할 수 있습니다.
          </Typography>
        </Box>
      </Grid>
      <Grid xs={0} md={4}></Grid>
      ...
      <Grid xs={12} md={12} sx={{ marginTop: "30px" }}>
        <Box sx={{ width: "100%", textAlign: "left" }}>
          <Typography variant="body1" gutterBottom>
            ...
            복사본을 생성할 수 있습니다. 변경 가능한 참조만 작성할 수 있습니다.
            쓰기 *x = v는 이전에 x에 저장된 값을 버리고 v로 업데이트합니다. 두
            작업 모두 C와 유사한 * 구문을 사용합니다. 그러나 읽기는 표현식인
            반면 쓰기는 등호의 왼쪽에서 발생해야 하는 변형입니다.
          </Typography>
        </Box>
      </Grid>
      <Grid xs={0} md={2}></Grid>
      ...
      <Grid xs={12} md={12} sx={{ marginTop: "30px" }}>
        <Box sx={{ width: "100%", textAlign: "left" }}>
          <Typography variant="body1" gutterBottom>
            참조를 읽으려면 기본 유형에 복사 기능이 있어야 합니다. 참조를 읽으면
            값의 새 복사본이 생성되기 때문입니다. 이 규칙은 리소스 값 복사를
            방지합니다.
          </Typography>
        </Box>
      </Grid>
      ...
      <Grid xs={12} md={12} sx={{ marginTop: "30px" }}>
        <Box sx={{ width: "100%", textAlign: "left" }}>
          <Typography variant="body1" gutterBottom>
            ...
            참조에 작성하면 이전 값이 삭제(또는 "삭제")되기 때문입니다. 이
            규칙은 리소스 값의 파괴를 방지합니다.
          </Typography>
        </Box>
      </Grid>
      <Grid xs={0} md={4}></Grid>
      ...
      <Grid xs={12} md={12} sx={{ marginTop: "30px" }}>
        <Box sx={{ width: "100%", textAlign: "left" }}>
          <Typography variant="body1" gutterBottom>
            이 동결 추론을 통해 Move 유형 검사기는 &mut T를 &T의 하위 유형으로
            볼 수 있습니다. 위에 표시된 것처럼 이것은 &T 값이 사용되는 모든
            표현식에 대해 &mut T 값도 사용할 수 있음을 의미합니다. 이 용어는
            &T가 제공된 곳에 &mut T가 필요했음을 간결하게 나타내기 위해 오류
            메시지에 사용됩니다. 예를 들어
          </Typography>
        </Box>
      </Grid>
      ...
      <Grid xs={12} md={12} sx={{ marginTop: "30px" }}>
        <Box sx={{ width: "100%", textAlign: "left" }}>
          <Typography variant="body1" gutterBottom>
            참조와 튜플은 구조체의 필드 값으로 저장할 수 없는 유일한 유형이며,
            이는 전역 저장소에 존재할 수 없음을 의미하기도 합니다. 프로그램 실행
            중에 생성된 모든 참조는 Move 프로그램이 종료될 때 삭제됩니다. 그들은
            완전히 일시적입니다. 이 불변성은 저장 기능이 없는 유형의 값에도
            적용되지만 참조와 튜플은 애초에 구조체에서 허용되지 않음으로써 한
            단계 더 나아갑니다. 이것은 Move와 Rust의 또 다른 차이점으로, 구조체
            내부에 참조를 저장할 수 있습니다. 현재 Move는 참조를 직렬화할 수
            없기 때문에 이를 지원할 수 없지만 모든 Move 값은 직렬화 가능해야
            합니다. 이 요구 사항은 프로그램 실행 간에 값을 유지하기 위해 값을
            직렬화해야 하는 Move의 영구 전역 저장소에서 비롯됩니다. 구조체는
            전역 저장소에 쓸 수 있으므로 직렬화 가능해야 합니다. 참조를 구조체에
            저장하고 해당 구조체가 전역 저장소에 존재하는 것을 금지하는 더
            멋지고 표현력이 뛰어난 유형 시스템을 상상할 수 있습니다. 저장 기능이
            없는 구조체 내부의 참조를 허용할 수도 있지만 문제를 완전히
            해결하지는 못합니다. Move에는 정적 참조 안전성을 추적하는 상당히
            복잡한 시스템이 있으며 유형 시스템의 이러한 측면도 확장해야 합니다.
            구조체 내부에 참조 저장을 지원합니다. 요컨대, Move의 유형
